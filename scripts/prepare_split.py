# prepare_split.py splits/dataset.txt
# Downloads and sets up ./PDBs/*

import os
import requests
import argparse
from helpers import pdb_sequence_maker
...
def download_pdb(pdb_id):
    ...
    if os.path.exists(pdb_output_path):
        print(f"[prepare_split] PDB already exists: {pdb_output_path}")
        return True

    if download_file(pdb_redo_url, pdb_output_path):
        print(f"[prepare_split] Successfully downloaded PDB from PDB_REDO: {pdb_id}")
        ...
        if download_file(mtz_url, mtz_output_path):
            print(f"[prepare_split] Successfully downloaded MTZ from PDB_REDO: {pdb_id}")
        else:
            print(f"[prepare_split] Failed to download MTZ from PDB_REDO: {pdb_id}")
    else:
        print(f"[prepare_split] PDB_REDO not available for {pdb_id}, trying RCSB PDB...")
        rcsb_url = f"https://files.rcsb.org/download/{pdb_id}.pdb"
        
        if download_file(rcsb_url, pdb_output_path):
            print(f"[prepare_split] Successfully downloaded PDB from RCSB: {pdb_id}")
        else:
            print(f"[prepare_split] Failed to download PDB for {pdb_id} from all sources")
            return False
...
def main():
    ...
    try:
        ...
        print(f"[prepare_split] Split file not found: {args.split_file}")
        return
    
    print(f"[prepare_split] Downloading {len(pdb_ids)} PDB structures...")
    ...
    for pdb_id in pdb_ids:
        if download_pdb(pdb_id):
            ...
            if (len(thisPDBSeqArray) > 1):
                print(f"[prepare_split] Multiple sequences found for {pdb_id}, using the first one.")
            if usingSequence:
                ...
                print(f"[prepare_split] Sequence for {pdb_id} saved to {seq_file_path}")
            ...
            if get_rid_of_water_and_ions(new_original_path, cleaned_pdb_path):
                print(f"[prepare_split] Cleaned PDB for {pdb_id} saved to {cleaned_pdb_path}")
            else:
                print(f"[prepare_split] Failed to clean PDB for {pdb_id}")
        current_iter += 1
        print(f"[prepare_split] Progress: {current_iter}/{total_iter} ({(current_iter / total_iter) * 100:.2f}%)")
    print("[prepare_split] All tasks completed.")
