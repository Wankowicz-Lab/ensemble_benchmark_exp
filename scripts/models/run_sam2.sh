...
OUTPUT_FILE="${PDB_DIR}/${PDB_ID,,}_sam2.pdb"
...
python ./scripts/generate_ensemble.py -c ./config/mdcath_model.yaml -i $ABSOLUTE_NOWAT_PDB_PATH -o $ABSOLUTE_PDB_DIR/sam2 -n $2 -b 8 -T 320 -d cuda
echo "[run_sam2.sh] Inference completed for $PDB_ID"

cd ../..

if [ ! -f "$ABSOLUTE_PDB_DIR/sam2.traj.dcd" ]; then
    echo "[run_sam2.sh] Output file $ABSOLUTE_PDB_DIR/sam2.traj.dcd does not exist. Exiting."
    exit 1
fi
