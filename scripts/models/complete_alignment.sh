...
# REMOVE LOW OCCUPANCY ATOMS SO ATOM COUNT MATCHES!
python ./scripts/helpers/make_static_pdb.py "$PDB_ID"
TOPOLOGY_PATH="${PDB_DIR}/${PDB_ID}_nowat_static.pdb"
###################################################

python ./scripts/helpers/align_with_mdtraj.py "$ESB_PATH" "$TOPOLOGY_PATH"


echo "[complete_alignment.sh] Alignment with MDTRAJ completed. Now aligning with PHASER..."
