...
mkdir -p "$INPUT_DIR/phenix_refine"
...
for MODEL_PATH in "$INPUT_DIR/models"/*.pdb; do
    ...
    mkdir -p "$INPUT_DIR/models/phenix_refine_${MODEL_BASENAME}"


    ABS_MODEL_PATH=$(realpath "$MODEL_PATH")
    ABS_MTZ_PATH=$(realpath "$MTZ_FILE")

    cd "$INPUT_DIR/models/phenix_refine_${MODEL_BASENAME}"


    phenix.refine $ABS_MTZ_PATH $ABS_MODEL_PATH main.number_of_macro_cycles=8 strategy=rigid_body space_group="P 21 21 21"
    cd - 
    cp "$INPUT_DIR/models/phenix_refine_${MODEL_BASENAME}/${MODEL_BASENAME}_refine_001.pdb" "$INPUT_DIR/phenix_refine/${MODEL_BASENAME}.pdb"
   

    echo "Finished processing: $MODEL_PATH"
done

OUTPUT_PDB="$INPUT_DIR/phenix_refined.pdb"
for REFINED_MODEL in "$INPUT_DIR/phenix_refine"/*.pdb; do
